...
pub fn startup_load_system(
    ...
    if let Err(e) = try_load_and_apply(&mut grid, &mut inventory, &mut money, &mut owned, smelter_q, player_q) {
...
fn try_load_and_apply(
    ...
    let Ok(mut smelter) = smelter_q.get_single_mut() else {
        ...
        save::apply(data, grid, inventory, money, owned, &mut throwaway, &mut pos);
